...
function BedroomModel() {
    ...
    useEffect(() => {
        ...
        scene.traverse((child) => {
            ...
            // These are typically named with 'hotspot', 'annotation', 'marker', or numbers
            ...
            if (
                ...
                name.includes('label') ||
                name.includes('point') ||
                /^\d+$/.test(child.name) || // Just numbers like "1", "2", "3"
                name.includes('sphere') && child.geometry?.type === 'SphereGeometry'
            ) {
                child.visible = false;
                console.log('Hiding annotation object:', child.name);
            }
        });
        ...
        console.log('Available bedroom animations:', names);
        names.forEach((name) => {
            if (actions[name]) {
                actions[name].play();
            }
